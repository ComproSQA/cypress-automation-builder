...
describe('Builder Regression Test',function(){
    ...
    it('Create an umbrella product with unique code starting from "cqa"', function(){
        ...
        */
        cy.get(`[title="${umbrellaUniqueCode}"]`).contains(`${umbrellaUniqueCode}`).should('be.visible').then(($result)=>{
            cy.get(`[title="${umbrellaUniqueCode}"]`).contains(`${umbrellaUniqueCode}`).click();
        })
    ...
    it('Create a component with a unique code starting from "cqa"', function(){        
        ...
        cy.get('.template-options-widget[role="option"]').find('.builder-carousel-widget').each(($el,index,$list)=>{
            var templateText=$el.find('.carousel-header').text();
            if(templateText.includes("Practice Extra"))
                {
                    cy.wrap($el).click()
                }
        })
